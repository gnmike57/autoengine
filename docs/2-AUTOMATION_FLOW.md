...
## Phase 4: Post-Attempt Instrumentation

After every login attempt (both initial and fast-loop), the following instrumentation runs automatically:

### 12. Response Screenshot Capture
Immediately after each attempt's return point, `ResponseScreenshotter.captureAttempt()` fires:
- **Zoom screenshot**: Element-level capture of the detected response banner (error message, success indicator)
- **Full-page screenshot**: Viewport capture with the response element highlighted via red CSS outline
- **Storage**: `screenshots/responses/{email}/{site}/{password}_attempt-{N}-{verdict}-{zoom|full}.webp`
- **Non-blocking**: Wrapped in `.catch(() => {})` — never blocks on screenshot failures

### 13. Headed Overlay Update
For headed (visual) mode, the browser overlay HUD is updated at every flow state change:
- Shows geo-verified IP address with Australian flag confirmation (AU ✅ / XX ❌)
- Displays unmasked email and current password
- Live session timer and attempt counter (e.g., 2/4)
- State-aware color coding (orange=cookie dismiss, blue=filling, pink=submitting, green=success, red=failed)

### 14. Hermes-Observer LLM Analysis (if available)
When OpenRouter API is configured, the Hermes-Observer runs real-time intelligence:
- **Live screenshot analysis**: Vision LLM reads the page screenshot and validates the code's verdict
- **Post-attempt analysis**: Combines screenshot + timing telemetry + attempt history for learning
- **Anomaly diagnosis**: Explains unexpected timing deviations and suggests corrections
- All intelligence persisted to `data/hermes-intelligence/live-observations.jsonl`

---

## Phase 5: Post-Batch Analysis

After a batch completes, the OpsOrchestrator runs timing analysis:
- **Statistical analysis**: Reads timing telemetry JSONL, computes P50/P95/max per flow phase
- **Proposal generation**: If any timing constant has >20% headroom (P95 × 1.3 < current value), generates a timing reduction proposal
- **LLM deep analysis**: Sends batch statistics to Gemini Flash for natural language summary with actionable improvements
- **Persistence**: All proposals saved to `data/hermes-proposals.json` for user review
