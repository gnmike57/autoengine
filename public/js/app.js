  ...
  if (id === 'analytics') { if (typeof updateAnalyticsChart === 'function') updateAnalyticsChart(); }
    ...
    function checkAnalysisButton(cfg) { const b = document.getElementById('btnViewAnalysis'); b.style.display = (cfg.isExperimental || cfg.backend === 'experimental') ? 'inline-flex' : 'none'; }
    ...
    function renderCredentialsTab() {
      ...
      document.getElementById('credPageInfo').textContent = `Showing ${start + 1}–${endIdx} of ${filtered.length}` + (filtered.length !== total ? ` (filtered from ${total})` : '');
