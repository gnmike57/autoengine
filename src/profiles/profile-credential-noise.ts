 ...
 */
export function getCanvasNoiseParams(emailOrSeed: string | number): CanvasNoiseParams {
  const seed = typeof emailOrSeed === "string" ? deriveNoiseSeedFromEmail(emailOrSeed) : emailOrSeed;
  return {
 ...
 */
export function getWebGLNoiseParams(emailOrSeed: string | number): WebGLNoiseParams {
  const seed = typeof emailOrSeed === "string" ? deriveNoiseSeedFromEmail(emailOrSeed) : emailOrSeed;
  return {
 ...
 */
export function getAudioNoiseParams(emailOrSeed: string | number): AudioNoiseParams {
  const seed = typeof emailOrSeed === "string" ? deriveNoiseSeedFromEmail(emailOrSeed) : emailOrSeed;
  return {
...
export function buildCredentialNoiseProfile(email: string): CredentialNoiseProfile {
  ...
  return {
    email,
    noiseSeed,
    canvas: getCanvasNoiseParams(noiseSeed),
    webgl: getWebGLNoiseParams(noiseSeed),
    audio: getAudioNoiseParams(noiseSeed),
  };
}
