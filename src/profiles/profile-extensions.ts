...
(function() {
  try {
    ...
    for (var i = 0; i < hints.length; i++) {
      var key = hints[i];
      if (!(key in window)) {
        try { Object.defineProperty(window, key, { value: true, configurable: true, writable: false, enumerable: false }); } catch { /* intentional */ }
      }
    }
    if (typeof window.chrome === 'object' && window.chrome) {
      if (!window.chrome.runtime) {
        try { Object.defineProperty(window.chrome, 'runtime', { value: { id: undefined, OnInstalledReason: { INSTALL: 'install', UPDATE: 'update' }, OnRestartRequiredReason: { APP_UPDATE: 'app_update' } }, configurable: true }); } catch { /* intentional */ }
      }
    }
    var ids = ${JSON.stringify(ids)};
    if (!('__installed_extension_ids' in window)) {
      try { Object.defineProperty(window, '__installed_extension_ids', { value: Object.freeze(ids.slice()), configurable: true, enumerable: false }); } catch { /* intentional */ }
    }
  } catch { /* intentional */ }
})();
  `;
